...
import InputIconField from "@/components/InputIconField/InputIconField.vue";
...
const passwordStyleOption = {
  ...
  input: {
    root: ({ props, state }) => ({
      class: [
        'w-full text-body3 h-[40px] shadow-none border border-[#E7E7E7] rounded-[4px] border-solid !pl-[38px] !pr-[46px] focus:outline-Primary placeholder:text-[#757575]',
        {
          '!border !border-[#F23C47] focus:border-[#F23C47]': props.invalid,
          'border border-[#E7E7E7] focus:border-Primary': !props.invalid,
        }
      ]
    }),
...
<template>
  <div v-if="item.type == FormItemType.NewPassword" class="flex flex-col gap-[24px] mb-[24px]">
    <InputIconField inputType="password" iconClass="absolute mt-[-12px]" :id="item.id" v-model:value="item.value"
      :feedback="false" :placeHolder="item.placeholder" :stylePt="passwordStyleOption" :invalid="item.invalid"
      @update:value="(val) => item.callback(item)">
      <template #icon><img src="@/assets/icon/Lock.svg" width="24" /></template>
    </InputIconField>
      ...
      <div class="flex gap-1 items-center" v-for="rule in item.rules">
        <span class="icon">
          <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"
            :class="{ 'fill-[#9B9B9B]': !rule.pass, 'fill-[#3ABC5D]': rule.pass }">
  ...
  <InputIconField v-if="item.type == FormItemType.CheckPassword" inputType="password" iconClass="absolute mt-[-12px]" :id="item.id" v-model:value="item.value"
    :feedback="false" :placeHolder="item.placeholder" :stylePt="passwordStyleOption" :invalid="item.invalid" :invalid-text="item.invalidText"
    @update:value="(val) => item.callback(val, item)">
    <template #icon><img src="@/assets/icon/Lock.svg" width="24" /></template>
  </InputIconField>
  <span class="text-[#F23C47] text-body3" v-if="item.invalid">{{ item.invalidText }}</span>
</template>