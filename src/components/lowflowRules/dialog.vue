...
const controller = new useDialog(t, locale, route, props.data, props.title, props.recordTypeList)
const emit = defineEmits(['eventClickSendBtn'])
const eventClickSendBtn = (type: boolean) => {
  if(type){
    if(props.title !== titleType.delete){
      const data = controller.checkAllData()
      emit('eventClickSendBtn', type, props.title, data)
    } else {
...
<template>
  <Dialog :blockScroll="true" :closable="false" v-model:visible="props.dialogShow" modal :header="$t(`Common.${props.title}`)" :class="{'w-[60%]':props.title !== titleType.delete}" class="w-[30%]">
    <div v-if="props.title !== titleType.delete" class="flex flex-col gap-[2rem]">
      <div v-for="(item, index) in controller.headerList.value" :key="index" class="flex items-center" :class="{'!hidden': !item.value}">
        ...
        <CustomFormItem v-if="!item.valueTypeIsArray" :data="item" class="w-[75%]" />
        <div class="w-[75%] flex flex-col gap-[1.5rem]" v-else>
          <div v-for="(item2, index2) in item.value" :key="index2" class="flex flex-col gap-[1.5rem]">
            <div v-for="(item3, index3) in controller.checkValueList(item, index2)" :key="index3" class="flex">
              <div class="w-[30%] flex items-center">{{ item3.label }}</div>
              <CustomFormItem :data="item3" class="w-[75%]" />
            </div>
          </div>
        </div>
      </div>
